...
export default function PaymentSuccessClient() {
  ...
  const [status, setStatus] = useState<
    "loading" | "activating" | "success" | "failed"
  >("loading");
  ...
  const fetchPaymentStatus = async (isRetry = false) => {
    ...
    try {
      ...
      const data = {
        ...
        isPaid:
          ...
          (rawData.status === "paid" ||
            ...
            rawData.status?.toUpperCase() === "PAID"),
        isFullySynced:
          rawData.isFullySynced ??
          (rawData.ucrm_sync_status === "completed" ||
            rawData.ucrm_sync_status?.toLowerCase() === "completed"),
      ...
      if (data.isFullySynced) {
        ...
        // Stop polling once fully successful
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      // Poll faster in the first few attempts
      const currentInterval = pollCountRef.current < 10 ? 3500 : 6000;

      fetchPaymentStatus(true);

      // Safety net: stop polling after max attempts if still activating
      if (pollCountRef.current >= maxPolls && status === "activating") {
        setErrorMsg(
          "Your payment is confirmed. Syncing is taking longer than expected. You can safely go to your dashboard.",
        );
        if (intervalRef.current) clearInterval(intervalRef.current);
      }
    }, 3500); // Consistent 3.5s polling for better responsiveness during testing
